...
aureii = {}
total_aureii = 20 #can't be lower than 10
...
def generate_coins():
    ...
    aureii[femme_hash] = femme_decentral
    ...
    gold_femme_id = randint(total_owner_managed+2,total_aureii-1)
    silver_femme_id = randint(total_owner_managed+2,total_aureii-1)
    ...
    for x in range(total_owner_managed+2, total_aureii):
        ...
        if x == gold_femme_id:
            ...
            aureii[gold_femme_hash] = gold_femme
            token_id += 1
        elif x == silver_femme_id:
            print('in silver')
            ...
            aureii[silver_femme_hash] = silver_femme
            token_id += 1
        else:
...
"""
Takes the raw dict of all aureii and turns it into the metadata json format that works for 
...
def generate_metadata():
    ...
    for aureus_key in aureii:

        # get the defintiion of this aureus's traits
        aureus_traits = aureii[aureus_key]
        ...
        if token_id == 1:
            aureus_metadata['description'] = "canonical description"
        elif token_id == 888:
            aureus_metadata['description'] = "femme decentral description"
        elif token_id == '∞':
            aureus_metadata['description'] = "memento description"
        elif token_id > 1 and token_id <= 7:
            aureus_metadata['description'] = "owner-managed description"
        elif aureus_traits['value'] == 'Believe. Build.':
            aureus_metadata['description'] = "azuki description"
        else:
            aureus_metadata['description'] = "regular token description"
        ...
        with open('metadata/' + str(token_id) + '.json', 'w') as f:
